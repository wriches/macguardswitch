...
Two sets of double-click launchers call the wrapper: `Connect VPN.app` / `Disconnect VPN.app` (primary — native admin prompt, no Terminal) and `Connect VPN.command` / `Disconnect VPN.command` (fallback — Terminal). The apps are built from the checked-in `*.applescript` sources by **`tools/build-apps.sh`**, which compiles them with `osacompile` *and* swaps osacompile's 455 KB default icon for the small custom `tools/{Connect,Disconnect}.icns` (~90 KB), then re-signs (ad-hoc). Always rebuild via that script — bare `osacompile` re-ships the bloated default icon. Edit the `.applescript`, run `tools/build-apps.sh`, and commit the regenerated bundles. The `.icns` are generated from SF Symbols by `tools/make-icons.swift` (needs Swift; only rerun if changing the icon art). Each app must sit next to `macguardswitch-vpn.sh` (it resolves the wrapper via `dirname` of `path to me`).
...
- **Launch environment is hostile — assume no tty, minimal PATH, maybe no exec bit.** The apps run the wrapper via `do shell script … with administrator privileges`, which gives **no controlling terminal**, a **stripped PATH** (no Homebrew/MacPorts → `wg`/`wg-quick`/`wireguard-go` missing), and surfaces only **stderr** in its error dialog. So: both scripts **prepend** `/opt/homebrew/bin:/usr/local/bin:/opt/local/bin` to PATH (prepend, not append — `wg-quick`'s `#!/usr/bin/env bash` needs bash 4+, and macOS's `/bin/bash` is 3.2, so Homebrew must come first or wg-quick aborts with "Version mismatch: bash 3 detected"; shadowing system tools isn't a real risk since Homebrew installs coreutils/grep/awk g-prefixed by default and our usage is POSIX); sibling scripts are invoked as `bash "$KS"` / `bash "$SELF"` (never relying on the exec bit, which zip/AirDrop strip); the supervisor is detached with **`trap '' HUP`** (NOT `nohup`, which needs a tty and dies with "Inappropriate ioctl for device"); and user-facing failure detail in `connect` goes to **stderr** so the dialog shows it. Don't reintroduce `nohup`, bare `"$SELF"`/`"$KS"` execution, or stdout-only error messages.
- **Supervisor lifecycle (wrapper).** `connect` launches a detached root supervisor (backgrounded + `trap '' HUP` + reparented to launchd) so closing the Terminal/window doesn't kill it. The supervisor owns the `arm` watcher (tracked via `ARM_PIDFILE`) and the tunnel, and polls whether the *initiating* user (uid → username) still has a GUI login session via `pgrep -u <user> -x loginwindow`. This is true during fast user switching (session stays loaded) and false on full logout — that distinction is the whole point, so don't replace it with a console-user or `who` check. On logout/SIGTERM it tears down (stop watcher → `wg-quick down` → `disarm`), staying fail-closed until `disarm`. A `disarm`-on-`EXIT` trap is still forbidden — teardown runs only on intentional signals/logout, so an abnormally-killed supervisor leaves the kill-switch up.
