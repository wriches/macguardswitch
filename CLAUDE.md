...
- **Launch environment is hostile — assume no tty, minimal PATH, maybe no exec bit.** The apps run the wrapper via `do shell script … with administrator privileges`, which gives **no controlling terminal**, a **stripped PATH** (no Homebrew/MacPorts → `wg`/`wg-quick`/`wireguard-go` missing), and surfaces only **stderr** in its error dialog. So: both scripts append `/opt/homebrew/bin:/usr/local/bin:/opt/local/bin` to PATH; sibling scripts are invoked as `bash "$KS"` / `bash "$SELF"` (never relying on the exec bit, which zip/AirDrop strip); the supervisor is detached with **`trap '' HUP`** (NOT `nohup`, which needs a tty and dies with "Inappropriate ioctl for device"); and user-facing failure detail in `connect` goes to **stderr** so the dialog shows it. Don't reintroduce `nohup`, bare `"$SELF"`/`"$KS"` execution, or stdout-only error messages.
